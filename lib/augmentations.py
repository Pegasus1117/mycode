...
class Normalize(object):
    ...
    Normalize the image
    """
    def __init__(self, mean, stds, depth_mean, depth_std, use_rcnn_pretrain):
...
class Resize(object):
    ...
    def __init__(self, size):
        self.size = size        # [512, 1760]

    def __call__(self, image, depth, imobj=None):

        scale_factor = self.size[0] / image.shape[0]    # 尺度变化因子是以高计算得到，在变化宽时会产生尺寸有+-1的不准

        h = np.round(image.shape[0] * scale_factor).astype(int)
        w = np.round(image.shape[1] * scale_factor).astype(int) # 最后一步取整操作可能会导致尺寸有+-1的不准，需要后面剪裁
...
class RandomMirror(object):
    """
    Randomly mirror an image horzontially, given a mirror probabilty.  随机水平翻转

    Also, adjust all box cordinates accordingly.
    """
    def __init__(self, mirror_prob):
        self.mirror_prob = mirror_prob  # 0.5
...
class Augmentation(object):
    ...
    def __init__(self, conf):
        ...
        self.distort_prob = conf.distort_prob               # -1
...
class Preprocess(object):
    ...
    """
    def __init__(self, size, mean, stds, depth_mean, depth_std, use_rcnn_pretrain):

...
def generate_anchors(conf, imdb, cache_folder):
    ...
    # generate anchors
        ...
        anchors = np.zeros([len(conf.anchor_scales)*len(conf.anchor_ratios), 4], dtype=np.float32)  # (36,4)

        aind = 0

        # compute simple anchors based on scale/ratios
        # ======1.由给定的尺度和比例生成各类anchors======
        ...
        for scale in conf.anchor_scales:
            ...
            for ratio in conf.anchor_ratios:
                ...
                anchors[aind, 0:4] = anchor_center(w, h, conf.feat_stride)  # anchors的前四个值存放的为anchor框的对角坐标
                '''特别注意这里的conf.feat_stride，它确定了这些anchor框的中心点坐标，因为使用同一个值，所以这些anchor框的中心点坐标相同'''
                aind += 1


        # optionally cluster anchors
        if conf.cluster_anchors:        # cluster_anchors = 0
        ...
        # ======2.由标签值中的真实值生成2d和3d的box信息并存在一起======
        elif conf.has_3d:       # has_3d = True

            # compute the default stats for each anchor
            normalized_gts = []

            # check all images
            # 这里处理的对象为标签值，根据标签值计算2d和3d的box并存在一起
            for imind, imobj in enumerate(imdb):

                # has ground truths?
                if len(imobj.gts) > 0:

                    scale = imobj.scale * conf.test_scale / imobj.imH   # = 1*512/370=1.38378378...

                    # determine ignores 确定要忽略的目标
                    igns, rmvs = determine_ignores(imobj.gts, conf.lbls, conf.ilbls, conf.min_gt_vis,
                                                   conf.min_gt_h, np.inf, scale)    # 返回值为布尔索引列表

                    # accumulate boxes
                    # 标签值中2d box信息
                    gts_all = bbXYWH2Coords(np.array([gt.bbox_full * scale for gt in imobj.gts]))   # Convert from [x,y,w,h] to [x1, y1, x2, y2]
                    gts_val = gts_all[(rmvs == False) & (igns == False), :]     # 滤掉可忽略的目标
                    # 标签值中3d box信息
                    gts_3d = np.array([gt.bbox_3d for gt in imobj.gts])  # obj.bbox_3d = [cx, cy, cz3d_2d, w3d, h3d, l3d, alpha, cx3d, cy3d, cz3d, rotY]
                    gts_3d = gts_3d[(rmvs == False) & (igns == False), :]       # 滤掉可忽略的目标

                    if gts_val.shape[0] > 0:

                        # center all 2D ground truths
                        for gtind in range(0, gts_val.shape[0]):
                            w = gts_val[gtind, 2] - gts_val[gtind, 0] + 1   # 这里的w等于上面的gt.bbox_full[2] * scale; 即真实值中的w*scale
                            h = gts_val[gtind, 3] - gts_val[gtind, 1] + 1

                            gts_val[gtind, 0:4] = anchor_center(w, h, conf.feat_stride)
                            '''注意这里也使用了conf.feat_stride，这意味着和上面产生的simple anchors中心点坐标相同，anchors框的长宽不同；也就是说，
                            把由真实值确定长和宽的anchors框平移到和之前产生的simple anchors中心点相同，然后进行比较，以得到最匹配的anchors'''

                    if gts_val.shape[0] > 0:
                        # 将２d和３d合在一起；每一个有效的目标增加一个，这意味着len(normalized_gts)=有效目标数(11956)；
                        # gts_val维度为4，gts_3d维度为11，合起来为15；
                        normalized_gts += np.concatenate((gts_val, gts_3d), axis=1).tolist()

            # ======3.处理数据的格式，然后每个真实值找到最佳匹配的anchor======
            # convert to np
            normalized_gts = np.array(normalized_gts)   # 真实值生成box; 形状为（11956,15）

            # expand dimensions
            anchors = np.concatenate((anchors, np.zeros([anchors.shape[0], 5])), axis=1) # anchors的前四个值为anchor框的坐标，后5个值初始化为0，待后面更新；
            ...
            ols = iou(anchors[:, 0:4], normalized_gts[:, 0:4])  # 计算所有的iou;每一列的值为同一个真实值对应的不同(共36个)anchors的iou；形状为（36,11956）
            gt_target_ols = np.amax(ols, axis=0)        # 返回的为数值
            gt_target_anchor = np.argmax(ols, axis=0)   # 返回的为索引（anchor的索引）

            # assign each box to an anchor 为每个（真实值）box指定一个（最匹配的）anchor
            for gtind, gt in enumerate(normalized_gts):

                anum = gt_target_anchor[gtind]  # gtind代表每一个真实值；gt_target_anchor[gtind]代表单个真实值最匹配的anchors的索引值

                if gt_target_ols[gtind] > 0.2:
                    # 根据索引值，在相应位置填入对应的真实标签值;
                    # 以anchors_z3d[anum]为例，该位置的列表中有多个值，表示有多少个真实值最匹配的anchor的对应序号为anum;
                    # 列表中值的数量即表示这种anchor（种类号为anum）匹配到的真实值的数量，数量越多“可能”说明这种anchors更符合数据中的大多数目标；
                    # 而这里放入的是真实标签数据，后续的操作是取平均值；这个最后取的平均值，表示这种(序号为anum:0-35)anchors最匹配的真实值，即这种anchors和这个平均值的数据最匹配；
                    anchors_z3d[anum].append(gt[6])
                    anchors_w3d[anum].append(gt[7])
                    anchors_h3d[anum].append(gt[8])
                    anchors_l3d[anum].append(gt[9])
                    anchors_rotY[anum].append(gt[10])   #TODO 这里写的是rotY，但是实际指的好像是alpha！？
            ...
            # update anchors 更新anchors的后5个值（anchors的前四个值为anchor框的坐标）
            for aind in range(0, anchors.shape[0]):
                ...
                if len(np.array(anchors_z3d[aind])) > 0:
                    ...
                    if conf.has_3d:
                        ...
                        # 形状均为（11747,）,表明只有11747个anchors是有效的,略小于11956;
                        # 这是因为上面程序中的gt_target_ols[gtind] > 0.2滤去了部分不太匹配的anchors
        ...
        if (cache_folder is not None):
            ...
            # anchors形状为（36,9）;36种anchor的种类，每种anchor有9个值，该值是其最匹配的真实值;
            # 9个值:[x1, y1, x2, y2, z3d, w3d, h3d, l3d, rotY];0-4个为anchor的对角坐标，5为‘深度’值，6-8为长宽高;
            # 根据程序的逻辑分析得出：每种anchor的值是其最匹配的真实值；
            # 从结果上来看也是如此:anchor的序号（3个3个跳）越靠后，框越大，与其匹配的真实目标的边界框越大，表明目标离相机距离越近，其z值（第5个）越小，结果也确实如此；

    conf.anchors = anchors  # 注意：在这里更新了conf.anchors
...
def compute_targets(gts_val, gts_ign, box_lbls, rois, fg_thresh, ign_thresh, bg_thresh_lo, bg_thresh_hi, best_thresh,
    ...
    """
    # ==========0. 初始化==========
    ols = None                  # 结合后面，ols可能为overlaps的简写，表示重叠情况
    has_3d = gts_3d is not None     # True
    use_corner = gts_vertices is not None   # False
    ...
    if use_corner:
        ...
        transforms = np.pad(transforms, [(0, 0), (0, gts_3d.shape[1])], 'constant')  # 周围填充；
        # ‘上’、‘下’、’前‘填充宽度均为0，‘后’填充宽度均为11，填充数为默认（0）;(122112,5)->(122112,16)
        raw_gt = np.pad(raw_gt, [(0, 0), (0, gts_3d.shape[1])], 'constant')  # 变化完全同上
    # ==========1. 计算变换（或者说准确率） ==========
    if gts_val.shape[0] > 0 or gts_ign.shape[0] > 0:
    # ==========1.1 计算忽略目标的重叠率（iou_ign）==========
    ...
    # ==========1.2 计算有效目标的重叠率（iou）==========
        if gts_val.shape[0] > 0:
    # ==========1.2.1 计算所有感兴趣区域与有效真实值的重叠率，并从两个角度找到的重叠率最大对应的感兴趣区域==========

            # compute overlaps valid 计算所有感兴趣区域（anchors）与有效真实值的重叠率，并获取每个感兴趣区域最匹配的真实值的值及其索引；
            ols = iou(rois, gts_val)    # (122112,有效目标数)
            ols_max = np.amax(ols, axis=1)  # (122112,),返回的是沿axis=1轴上的最大值；
            targets = np.argmax(ols, axis=1)    # (122112,),返回的是沿axis=1轴上的最大值的索引；其值域为有效目标的索引值（0~有效目标数-1）
            '''注意axis=1与axis=0的区别，同样是取最大值及其索引，但其表现的意义不同：
            axis=1是找所有''感兴趣区域（anchors）''最大的有效真实值的重叠区域最大的值及其索引,为感兴趣区域找最匹配的真实值,为前景
            axis=0是找所有''有效真实值''最大的感兴趣区域（anchors）的重叠区域最大的值及其索引,为真实值找最匹配的感兴趣区域'''

            # find best matches for each ground truth       为每个真实值找最匹配的感兴趣区域
            gt_best_rois = np.argmax(ols, axis=0)   # 索引;形状：(有效目标数，)，其值域为感兴趣区域的索引值（0~122111）
            gt_best_ols = np.amax(ols, axis=0)      # 值

            gt_best_rois = gt_best_rois[gt_best_ols >= best_thresh]  # 过滤感兴趣区域;只有当最大值大于best_thresh时才保留，滤掉了与真实值boxes重叠区域不够大的感兴趣区域
            gt_best_ols = gt_best_ols[gt_best_ols >= best_thresh]

    # ==========1.2.2 将从两种角度找到的重叠区域最大对应的感兴趣区域（anchors）进行了汇总整合==========
            fg_inds = np.flatnonzero(ols_max >= fg_thresh)  # 过滤的还是感兴趣区域;返回的为索引,形状：(满足要求的数量,)；其目的为找含目标大部分区域的区域，即图像中含有较完整目标的前景区域
            fg_inds = np.concatenate((fg_inds, gt_best_rois))   # 形状：(满足要求的数量,) -> (满足要求的数量+有效目标数,)
            fg_inds = np.unique(fg_inds)    # 返回的为索引；去掉重复的区域，并排序输出感兴趣区域（anchors）编号；形状：(满足要求的数量+有效目标数-重复对数,)

            '''target_rois把 汇总整合的感兴趣区域（anchors）编号所对应的重叠最大的目标的真实值boxes数据 复制进来；数据来源为真实值boxes
            src_rois把 汇总整合的感兴趣区域（anchors）编号所对应的感兴趣区域的anchors框数据 复制进来；数据来源为感兴趣区域的anchors框数据'''
            target_rois = gts_val[targets[fg_inds], :]  # (满足要求的数量+有效目标数-重复对数,4)；此处用了一种高级索引；
            src_rois = rois[fg_inds, :] # (满足要求的数量+有效目标数-重复对数,5)

            if len(fg_inds) > 0:
    # ==========1.2.3 计算2Dtransform==========
                # compute 2d transform  计算2D中src_rois与target_rois的bbox的变换情况。
                transforms[fg_inds, 0:4] = bbox_transform(src_rois, target_rois) #　将２Ｄ变换情况（dx,dy,dw,dh）存到transforms中对应编号行的前４个位置(第１-４）;

                raw_gt[fg_inds, 0:4] = target_rois  # 0~3存对应的真实值２Ｄ数据；(122112,16)
    ...
    # ==========1.2.4 计算3Dtransform==========
                    tracker = tracker.astype(np.int64)  # (122112,)
                    src_3d = anchors[tracker[fg_inds], 4:]  # (整合后数量,5)；感兴趣区域（anchors）数据中的后5个３Ｄ数据；
                    target_3d = gts_3d[targets[fg_inds]]    # (整合后数量,11)；真实值３Ｄ数据；

                    raw_gt[fg_inds, 5:] = target_3d         # 5~15存对应的真实值３Ｄ数据；

                    # compute 3d transform
                    # 3Ｄ变换情况为：计算了７项变化(dx,dy,dz,dw,dh,dl,deltaRotY)，其余４项未计算；
                    # 将3Ｄ变换情况存到transforms中对应编号行的后11个位置（第6-16）;
                    transforms[fg_inds, 5:] = bbox_transform_3d(src_rois, src_3d, target_3d)

    # ==========1.2.5 储存类型编号==========
                # store labels  储存类型编号
                transforms[fg_inds, 4] = [box_lbls[x] for x in targets[fg_inds]]    # 将3Ｄ变换情况存到transforms中对应编号行的第５个位置;
    ...
    # ==========1.3 标记后景类型 ==========
        ...
        # determine background  重叠率在bg_thresh_lo和bg_thresh_hi之间感兴趣区域认定为后景
    ...
    '''
    返回值说明：
    transforms：（122112,16）;真实值boxes与感兴趣区域框（rois）重叠在设定阈值之上的rois的编号所对应的行（有效行）中才有数据，其余行全为0；
                有效行中的数据：1-4:(dx,dy,dw,dh),5:类型编号,6-16:(dx,dy,dz,dw,dh,dl,deltaRotY)+(cx3d, cy3d, cz3d, rotY)
        transforms的意义？通过分析程序看出： 1.筛选出一批与ground truth有较大重叠率的感兴趣区域；
                                        2.同时，算出了这些rois与ground truth的‘相对误差’；
                                        3.学术叫法为：bbox regression
    ols：(122112,,有效目标数);所有感兴趣区域（rois）与有效目标的真实值boxes的重叠率；
    raw_gt：(122112,16);第５列(表类型)全为0，有效行以外全为0，有效行中的数据为该编号对应感兴趣区域框最匹配的真实值的数据；
    '''
...
def clsName2Ind(lbls, cls):
    """
    Converts a cls name to an ind   返回的类别对应编号;conf.lbls = ['Car', 'Pedestrian', 'Cyclist']，分别对应编号：1,2,3
    """
    if cls in lbls:
...
def compute_bbox_stats(conf, imdb, cache_folder=''):
    ...
    """
    '''
    平均值的含义：所有有用的rois相对于对应的ground truth相对误差的平均值；
    方差值同理；
    表现了生成的rois对ground truth的匹配（合适）程度；
    '''
    ...
    # ==========计算平均值==========
        ...
        for imind, imobj in enumerate(imdb):
            if len(imobj.gts) > 0:
        # ==========1.得到所有感兴趣区域（rois）==========
                scale_factor = imobj.scale * conf.test_scale / imobj.imH    # = 1*512/370=1.38378378...
                feat_size = calc_output_size(np.array([imobj.imH, imobj.imW]) * scale_factor, conf.feat_stride)  # feat_size=[32,106]；表示滑动次数
                rois = locate_anchors(conf.anchors, feat_size, conf.feat_stride)    # 得到各种anchors扩展到整个特征图上的框的坐标及种类;(122112,5);所有感兴趣区域（rois）
                '''locate_anchor输入的conf.anchors是之前在generate_anchors()更新后的值；'''

        # ==========2.确定要忽略的目标、有效目标及其boxes（2D+3D）与种类编号==========
                # determine ignores
                igns, rmvs = determine_ignores(imobj.gts, conf.lbls, conf.ilbls, conf.min_gt_vis,
                                               conf.min_gt_h, np.inf, scale_factor)

                # accumulate boxes  积聚单帧图像中所有目标的boxes       形状：（所有目标数，4）
                gts_all = bbXYWH2Coords(np.array([gt.bbox_full * scale_factor for gt in imobj.gts]))    # Convert from [x,y,w,h] to [x1, y1, x2, y2]

                # filter out irrelevant cls, and ignore cls
                gts_val = gts_all[(rmvs == False) & (igns == False), :]     # 单帧图像中所有有效真实值，形状：（有效目标数，4）
                gts_ign = gts_all[(rmvs == False) & (igns == True), :]

                # accumulate labels 积聚单帧图像中所有目标的种类并编号
                box_lbls = np.array([gt.cls for gt in imobj.gts])   # 积聚单帧图像中所有目标的种类
                box_lbls = box_lbls[(rmvs == False) & (igns == False)]  # 过滤
                box_lbls = np.array([clsName2Ind(conf.lbls, cls) for cls in box_lbls])  # 返回的是目标类型对应的编号（1,2,3）
                ...
                if conf.use_corner:
                    ...
                    # accumulate 3d boxes   积聚单帧图像中所有有效目标的3d boxes
                    gts_3d = np.array([gt.bbox_3d for gt in imobj.gts])
                    gts_3d = gts_3d[(rmvs == False) & (igns == False), :]   # 形状：（有效目标数，11）

                    # rescale centers (in 2d)   将2d中心点坐标乘以尺度系数
                    for gtind, gt in enumerate(gts_3d):
                        gts_3d[gtind, 0:2] *= scale_factor

        # ==========3.找出与有效目标有较大重叠的感兴趣区域，并计算其‘相对误差’==========
                    # compute transforms for all 3d
                    transforms, _, _= compute_targets(gts_val, gts_ign, box_lbls, rois, conf.fg_thresh, conf.ign_thresh,
                                                    conf.bg_thresh_lo, conf.bg_thresh_hi, conf.best_thresh, gts_3d=gts_3d,
                                                    anchors=conf.anchors, tracker=rois[:, 4])   # （122112,16）;
                    '''transforms的意义？通过分析程序看出： 1.筛选出一批与有效目标的ground truth有较大重叠率的感兴趣区域；
                    2.同时，算出了这些rois与ground truth的‘相对误差’；'''
                else:
        ...
        # ==========４.统计相关数据计算均值==========
                # 根据类型编号提取出 有效行（或者说，有效的rois）的索引；
                # 其中有效行(rois)指的是：图像中含有效目标（Car,Pedestrian,Cyclist），与这些目标的真实值boxes有较大重叠率的感兴趣区域框（rois）的编号。
                # 有效行中的数据主要是这些rois与ground truth的‘相对误差’
                gt_inds = np.flatnonzero(transforms[:, 4] > 0)  # (有效rois数,)
                ...
                if len(gt_inds) > 0:
                    ...
                    if conf.use_corner:
                        ...
                        sums[:, 0:4] += np.sum(transforms[gt_inds, 0:4], axis=0)    # 2D; transforms中2D计算了4项差异
                        sums[:, 4:] += np.sum(transforms[gt_inds, 5:12], axis=0)    # 3D; transforms中3D计算了7项差异
                    else:
                        sums += np.sum(transforms[gt_inds, 0:4], axis=0)
                    #　统计有效rois的数量
                    class_counts += len(gt_inds)
        # 求平均值；平均值的含义：所有有用的rois相对于对应的ground truth相对误差的平均值；表现了生成的rois对ground truth的匹配（合适）程度；
    ...
    conf.bbox_means = means     # 注意：更新了conf.bbox_means的值
    conf.bbox_stds = stds
...
def project_3d(p2, x3d, y3d, z3d, w3d, h3d, l3d, ry3d, return_3d=False):
    ...
    Args:
        p2 (nparray): projection matrix of size 4x3(应该是4x4)
    ...
    corners_3D_1 = np.vstack((corners_3d, np.ones((corners_3d.shape[-1]))))     # 由（3,8）变为（4,8）
    corners_2D = p2.dot(corners_3D_1)
    corners_2D = corners_2D / corners_2D[2]  # normalize dim 3 -> 2, image coordinate

    bb3d_lines_verts_idx = [0, 1, 2, 3, 4, 5, 6, 7, 0, 5, 4, 1, 2, 7, 6, 3]

    verts3d = (corners_2D[:, bb3d_lines_verts_idx][:2]).astype(float).T     # corners_2D[:, bb3d_lines_verts_idx][:2]包含两步索引（切片）
...
def bbox_transform_3d(ex_rois_2d, ex_rois_3d, gt_rois):
    ...
    deltaRotY = gt_rois[:, 6] - ex_rois_3d[:, 4]    #TODO 这里的是rotY，但是实际指的好像是alpha！？
    ...
    targets = np.hstack((targets, gt_rois[:, 7:]))  # 只计算前７项变换，后4项未计；
...
def bbox_transform(ex_rois, gt_rois):
    """
    Compute the bbox target transforms in 2D.   计算2D中的bbox目标变换。
    ...
    targets_dx = (gt_ctr_x - ex_ctr_x) / ex_widths  # anchors框大小不变，两中心点相距越远，dx的绝对值越大；两中心点相距不变，anchors框越大，dx的绝对值越小；
    targets_dy = (gt_ctr_y - ex_ctr_y) / ex_heights # 与上同理
    targets_dw = np.log(gt_widths / ex_widths)  # anchors框>真实boxes框，dw<0；anchors框<真实boxes框，dw<0；nchors框=真实boxes框，dw=0；
    targets_dh = np.log(gt_heights / ex_heights)    # 与上同理
...
def determine_ignores(gts, lbls, ilbls, min_gt_vis=0.99, min_gt_h=0, max_gt_h=10e10, scale_factor=1):
    ...
    根据给定的设置，决定真实值（ground truths）中那些目标是要忽略和移除的。对象为：单帧图像中的目标，返回值为布尔索引列表
    """
    ...
    for gtind, gt in enumerate(gts):
        ...
        ign = gt.ign                            # 车后面的直接忽略
        ign |= gt.visibility < min_gt_vis       # gt.visibility<0.65时忽略，即大部分遮挡（occlusion = 2,3）的目标忽略
        ign |= gt.bbox_full[3] * scale_factor < min_gt_h    # 2d边界框高度太小的忽略
        ign |= gt.bbox_full[3] * scale_factor > max_gt_h    # 2d边界框高度太大的忽略
        ign |= gt.cls in ilbls                              # 目标类型为'Van(货车)'，'ignore'的忽略

        rmv = not gt.cls in (lbls + ilbls)   # 如果目标种类不在'Van', 'ignore', 'Car', 'Pedestrian', 'Cyclist'之中，则remove

        igns[gtind] = ign       # ign = True , False
        rmvs[gtind] = rmv       # rmv = True , False
...
def locate_anchors(anchors, feat_size, stride, convert_tensor=False):
    ...
    # compute rois
    # ======1.根据滑动步长得到滑动次数，再结合修正后的图像尺寸，在特征图(feature map)中确定每次滑动的中心坐标======
    # shift_x和shift_y存储的是每次滑动的(乘尺度因子后的)起始（中心）坐标；
    # 数字的意义：每次滑动的步长为16,x方向滑动了106次,y方向滑动了32次
    shift_x = np.array(range(0, feat_size[1], 1)) * float(stride)
    shift_y = np.array(range(0, feat_size[0], 1)) * float(stride)
    [shift_x, shift_y] = np.meshgrid(shift_x, shift_y)  # 形状变化：（106,）->(32,106);(32,)->(32,106)

    rois = np.expand_dims(anchors[:, 0:4], axis=1)  # 形状：(36,1,4)；提取anchors的二维坐标；
    shift_x = np.expand_dims(shift_x, axis=0)       # 形状：(32,106)->(1,32,106)
    shift_y = np.expand_dims(shift_y, axis=0)

    # ======2.根据得到的中心坐标，结合提取的anchors框的坐标，得到各类anchors(每次滑动的)感兴趣区域（anchors框）的坐标======
    '''联系到之前的内容：之前的步骤虽然得到了anchors框坐标，但只是相对于同一个中心点的框的坐标，主要突出各种种类框的尺寸；
       到此，给定了遍布整个特征图的中心点；两者结合得到各类anchors的感兴趣区域的坐标框，有关anchors的生成这才算完满.'''
    # 形状：rois[:, :, 0]的shape为(36,1);  (1,32,106)+(36,1,1)->(36,32,106);
    # 数字的意义：36为anchors的种类,32为y方向滑动了32次,106为x方向滑动了106次;
    ...
    stack_size = feat_size[0] * anchors.shape[0]    # =32*36=1152
    ...
    if convert_tensor:
        ...
        # (36,32,106)->(1, 32*36=1152, 106)->(32*36*106=122112, 1)---->(122112,5)
        ...
        # 结果的数据形式：
        # rois[0]、[2]每加  1  个数据变一次，每加  106   个数据循环一遍；
        # rois[1]、[2]每加 106 个数据变一次，每加  3392  个数据循环一遍；3392=32*106
        # rois[4]    每加 3392 个数据变一次，每加 122112 个数据循环一遍；
        # 说明：先变化x值，（x值每变106次）再变化y值，（y值每变32次）最后变化anchors种类值；可以用时钟的秒针、分针、时针来理解；
        # 其物理意义的解释：在特征图上，先对于第一种anchors，anchor框从左到右横向移动，移完后进入下一行继续从左到右移动，
        #               直到便利完整个特征图，进入下一种anchors继续同样的操作，直至全部anchors.
        # 上面是理解方式；为什么数据是这样变化的？以shift_x1、shift_x2为例，其数据源头shift_x(32,106)就是只有(106个)列值有变化，行值都一样，与其物理意义一致。

    return rois     # 最后得到的rois是什么？是感兴趣区域（roi),是各种anchors扩展到整个特征图上的框的坐标及种类:[x1, y1, x2, y2, anchor_index]
...
def im_detect_3d(im, depth, net, rpn_conf, preprocess, p2, gpu=0, synced=False):
    ...
    im, depth = preprocess(im, depth)   # 图像尺寸、通道意义发生了变化；
...
def test_kitti_3d(dataset_test, test_split, net, rpn_conf, results_path, test_path, use_log=True):
    ...
    具体：
    dataset_test:'kitti_split1';    test_split:'validation';    rpn_conf:depth_guided_config.conf;
    results_path:./output/depth_guided_config/<result_dir>/results/results_xxxxx/data;   test_path:./data
    """
    ...
    # fix paths slightly     从结果路径中提取出当前test基于的迭代次数
...
def parse_kitti_result(respath, mode='new'):
    ...
    if mode == 'old':
        easy = np.mean(acc[0, 0:41:4])      # # 从索引 0 开始到索引 41 停止，间隔为 4;

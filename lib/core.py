...
def init_config(conf_name):
    ...
    # importlib.import_module()为动态导入对象(depth_guided_config)；后面的.Config()表示导入depth_guided_config下的Config()函数
...
def init_training_model(conf, cache_folder, conf_name):
    ...
    # ======1.加载模型，设置训练模式======
    # load and build
    network = absolute_import(dst_path)     # 加载模型;
    network = network.build(conf, 'train')  # 执行模型中的build方法;这里network.build中的.build是模型resnet_dilate.py中定义的函数

    # ======2.多GPU并行运算======
    # multi-gpu
    network = torch.nn.DataParallel(network)    # 在模块级别实现数据并行
...
def adjust_lr(conf, optimizer, iter, scheduler):
    ...
    if conf.solver_type.lower() == 'sgd':   # 'sgd'

        lr = conf.lr                # 0.01
        lr_steps = conf.lr_steps    # None
        max_iter = conf.max_iter    # 40000
        lr_policy = conf.lr_policy  # ='onecycle'  # 'onecycle'  # 'cosinePoly'  # 'cosineRestart'  # 'poly'
        lr_target = conf.lr_target  # = conf.lr * 0.00001
...
def intersect(box_a, box_b, mode='combinations', data_type=None):
    ...
    计算交集区域的面积的思路：
    核心思想是：得到相交区域（矩形）的一对对角点（左上，右下），然后计算相交区域（矩形）的面积。具体步骤如下：
    １.比较右下的坐标，分别取ｘ,y坐标最小值,得到相交区域的右下对角点（Ｂ）;
    2.比较左上的坐标，分别取ｘ,y坐标最大值,得到相交区域的左上对角点（Ａ）;
    3.用两个对角点坐标计算相交区域的面积。
     _________
    |   A ___|______
    |    |   |     |
...
def iou(box_a, box_b, mode='combinations', data_type=None):
    ...
    if mode == 'combinations':
        ...
        inter = intersect(box_a, box_b, data_type=data_type)     # 计算相交区域的面积
...
def init_training_paths(conf_name, result_dir, use_tmp_folder=None):
    ...
    具体来说：
    data    =  ./data
    output  =  ./output/depth_guided_config
    weights =  ./output/depth_guided_config/<result_dir>/weights
    logs    =  ./output/depth_guided_config/<result_dir>/log
    results =  ./output/depth_guided_config/<result_dir>/results
     <result_dir>类似于Adaptive_block2_resnet_dilate50_batch8_dropoutearly0_5_lr0_01_onecycle_iter40000_2020_11_19_15_53_35
    ...
    paths.data = os.path.join(paths.base, 'data')   # os.path.join() 函数用于路径拼接文件路径，可以传入多个参数。
...
def init_torch(rng_seed, cuda_seed):
    ...
    # make the code deterministic 使代码具有确定性
    torch.backends.cudnn.enabled = False # 原程序没有，加上是为了解决cuDNN error: CUDNN_STATUS_NOT_SUPPORTED的错误，这是禁用cudnn，会导致训练速度变慢。
...
def init_visdom(conf_name, visdom_port):
    ...
    """
    try:
        vis = visdom.Visdom(server='http://219.216.72.225', port=visdom_port, env=conf_name)    # 注意这里server要写本机的IP地址
